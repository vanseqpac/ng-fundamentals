...
//Always a good practice to make your services injectable
@Injectable()
export class EventService {

    //We want getEvents() to act asynchronously. To do that we use a 'rxjs' observable. 
    //Subject is a type of observable. We add data to the observable stream
    //Observables are streams of data. They are like arrays where the data arrives over time.
    //Add data to stream asynchronously (over time) after 100 millisecs
    getEvents():Observable<IEvent[]> {
      let subject = new Subject<IEvent[]>() //declare new rxjs Subject
      setTimeout(() => {subject.next(EVENTS); subject.complete();}, 100)  //to make getEvents() async. Add data to stream

      return subject  //return observable
    }
